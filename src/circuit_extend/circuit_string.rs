...
impl<F: PrimeField> CircuitString<F> {
    ...
    >(
        ...
        CircuitString::from_num_with_known_length(
            cs.ns(|| "generate CircuitString"),
            num,
            max_length,
        )
    }

    pub fn from_num_with_known_length<CS: ConstraintSystem<F>>(
        mut cs: CS,
        num: AllocatedFr<F>,
        max_length: usize,
    ) -> Result<Self, SynthesisError> {
        assert!(max_length <= F::Params::MODULUS_BITS as usize);
        let bits = get_bits_le_fixed(&num, cs.ns(|| "into_bits_le_fixed"), max_length)?;
        assert_eq!(bits.len(), max_length);

        let ce = CircuitString {
            num,
            le_bits: bits,
            length: max_length,
        };
