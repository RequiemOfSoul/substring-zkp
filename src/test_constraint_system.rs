...
use ckb_r1cs::{ConstraintSystem, Index, LinearCombination, SynthesisError, Variable};
...
impl<F: PrimeField> TestConstraintSystem<F> {
    ...
    pub fn pretty_print(&self) -> String {
        ...
        let powers_of_two = (0..F::size_in_bits())
            .map(|i| F::from(2u128).pow(&[i as u64]))
            .collect::<Vec<_>>();
    ...
    pub fn find_unconstrained(&self) -> String {
        ...
        let mut s = String::new();
        let pp = |hm: &mut HashSet<String>, lc: &LinearCombination<F>| {
            for (var, _coeff) in proc_lc::<F>(lc.as_ref()) {
                match var.0.get_unchecked() {
                    Index::Input(i) => {
